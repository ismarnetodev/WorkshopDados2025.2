...
def load_data():
    try:
        ...
        st.error(
            "Erro ao carregar o arquivo 'brasil.csv'. Verifique se o arquivo existe.")
        return None
...
if df is not None:
    ...
    with tab1:
        ...
        st.write(
            f"Acurácia do modelo de previsão de campeões: **{acc_campeoes*100:.2f}%**")
        ...
        chart_data = tabela_filtrada[[
            "time", "prob_campeao"]].set_index("time")
    ...
    with tab2:
        ...
        st.write(
            f"Acurácia do modelo de previsão de jogos: **{acc_jogo*100:.2f}%**")
        ...
        with col1:
            ...
            gols_casa = st.number_input(
                "Gols do time da casa", min_value=0, step=1, key="gols_casa")
            posse_bola_casa = st.slider(
                "Posse de bola da casa (%)", 0, 100, 50, key="posse_casa")
            chutes_casa = st.number_input(
                "Chutes do time da casa", min_value=0, step=1, key="chutes_casa")
            escanteios_casa = st.number_input(
                "Escanteios da casa", min_value=0, step=1, key="escanteios_casa")
            faltas_casa = st.number_input(
                "Faltas da casa", min_value=0, step=1, key="faltas_casa")

        with col2:
            st.markdown("**Time Visitante**")
            gols_visitante = st.number_input(
                "Gols do time visitante", min_value=0, step=1, key="gols_visitante")
            chutes_visitante = st.number_input(
                "Chutes do time visitante", min_value=0, step=1, key="chutes_visitante")
            escanteios_visitante = st.number_input(
                "Escanteios do visitante", min_value=0, step=1, key="escanteios_visitante")
            faltas_visitante = st.number_input(
                "Faltas do visitante", min_value=0, step=1, key="faltas_visitante")
